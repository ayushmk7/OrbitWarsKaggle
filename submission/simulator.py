...
# ---------------------------------------------------------------------------
# B4: board evaluator. Higher = better for `player`. Weights are tunable.
# ---------------------------------------------------------------------------
W_PROD_PER_TURN = 1.0   # production advantage compounds over remaining turns
W_SHIP = 1.0            # material
W_PLANET = 25.0         # board control
W_VULN = 2.0            # exposed-garrison penalty (anti lead-then-collapse)
TERMINAL_WIN = 1.0e7
...
def evaluate(state: GameState, player: int) -> float:
    if state.is_terminal():
        sc = state.scores()
        me = sc[player]
        best = max([sc[i] for i in range(len(sc)) if i != player], default=0)
        sign = 1.0 if me > best else (-1.0 if me < best else 0.0)
        return TERMINAL_WIN * sign + (me - best)

    my_prod = my_ships = my_planets = 0
    enemy = {}  # owner -> [prod, ships, planets]
    enemy_planet_xy = []
    for p in state.planets:
        o = p[P_OWNER]
        if o == player:
            my_prod += p[P_PROD]; my_ships += p[P_SHIPS]; my_planets += 1
        elif o >= 0:
            e = enemy.setdefault(o, [0, 0, 0])
            e[0] += p[P_PROD]; e[1] += p[P_SHIPS]; e[2] += 1
            enemy_planet_xy.append((p[P_X], p[P_Y], p[P_SHIPS]))
    for f in state.fleets:
        o = f[F_OWNER]
        if o == player:
            my_ships += f[F_SHIPS]
        elif o >= 0:
            enemy.setdefault(o, [0, 0, 0])[1] += f[F_SHIPS]

    if enemy:
        leader = max(enemy.values(), key=lambda e: e[0] * 1000 + e[1])
        e_prod, e_ships, e_planets = leader
    else:
        e_prod = e_ships = e_planets = 0

    remaining = max(0, state.episode_steps - state.step)
    value = (
        (my_prod - e_prod) * remaining * W_PROD_PER_TURN
        + (my_ships - e_ships) * W_SHIP
        + (my_planets - e_planets) * W_PLANET
    )

    # Vulnerability: my planets exposed to a larger nearby enemy garrison are
    # phantom assets. Penalising this makes search defend a lead rather than
    # collapse -- as a SOFT term search can trade off (unlike a hard reserve).
    if enemy_planet_xy:
        vuln = 0
        for p in state.planets:
            if p[P_OWNER] != player:
                continue
            px, py, g = p[P_X], p[P_Y], p[P_SHIPS]
            near_max = 0
            for ex, ey, es in enemy_planet_xy:
                if (ex - px) ** 2 + (ey - py) ** 2 < PREEMPTIVE_RANGE ** 2:
                    if es > near_max:
                        near_max = es
            if near_max > g:
                vuln += near_max - g
...
# ---------------------------------------------------------------------------
# B5: action-set construction from the greedy agent's candidate list.
...
def build_action_sets(base_decision: dict, max_sets: int = 16) -> list:
    ...
    defenses = [c["move"] for c in chosen if c.get("candidate_type") == "reinforce"]
    offense = [c for c in chosen if c.get("candidate_type") in ("expand", "attack", "comet")]
    consolidate = [c["move"] for c in chosen if c.get("candidate_type") == "consolidate"]
    offense.sort(key=lambda c: c.get("score", 0), reverse=True)
    offense_moves = [c["move"] for c in offense]

    sets = []  # (name, moves)
    sets.append(("do_nothing", []))
    sets.append(("greedy_full", defenses + offense_moves + consolidate))
    sets.append(("defend_only", list(defenses)))
    sets.append(("expand_only", defenses + [c["move"] for c in offense
                                            if c.get("candidate_type") in ("expand", "comet")]))
    sets.append(("attack_only", defenses + [c["move"] for c in offense
    ...
    # Dedup by move content; cap.
    seen = set()
    unique = []
    for name, moves in sets:
        key = tuple(sorted((m[0], round(m[1], 4), m[2]) for m in moves))
        if key in seen:
            continue
        seen.add(key)
        unique.append((name, moves))
...
# ---------------------------------------------------------------------------
# B5: the search entrypoint. Returns (moves, trace).
# ---------------------------------------------------------------------------
...
def search_decision(obs, time_budget_ms: float, base_decision: dict):
    ...
    # Always have a sane answer: greedy_full first, then do_nothing.
    order = sorted(range(len(action_sets)),
                   key=lambda i: 0 if action_sets[i][0] == "greedy_full"
                   else (1 if action_sets[i][0] == "do_nothing" else 2))

    best_name = "greedy_full"
    best_moves = next((m for n, m in action_sets if n == "greedy_full"), [])
    best_val = float("-inf")
    set_values = {}
    ...
    while True:
        for i in order:
            if elapsed_ms() > time_budget_ms * TIME_SAFETY:
                break
            name, moves = action_sets[i]
            val = rollout(state, moves, horizon, player)
            set_values[name] = val
            if val > best_val:
                best_val, best_name, best_moves = val, name, moves
        if elapsed_ms() > time_budget_ms * TIME_SAFETY or horizon >= MAX_HORIZON:
            break
        horizon += HORIZON_STEP
        deepened += 1
        best_val = float("-inf")  # re-rank at the deeper horizon

    trace = {
        "used": True,
        "chosen_set": best_name,
        "horizon": horizon,
        "deepened": deepened,
